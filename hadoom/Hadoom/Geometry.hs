...
makeSimple :: (Epsilon a,Fractional a,Ord a)
           => V.Vector (Point V2 a) -> V.Vector (Point V2 a) -> V.Vector (Point V2 a)
...
triangulate :: (Epsilon a,Fractional a,Ord a)
            => V.Vector (Point V2 a) -> V.Vector Int
...
triangleArea :: Fractional a => Point V2 a -> Point V2 a -> Point V2 a -> a
triangleArea a b c =
  let toV3 (P (V2 x y)) = V3 x y 1
      det =
        det33 (V3 (toV3 a)
                  (toV3 b)
                  (toV3 c))
...
data Circle a =
  Circle {circleCenter :: Point V2 a
         ,circleRadius :: a}
  deriving (Eq,Ord,Read,Show)

pointInTriangle :: (Epsilon a,Fractional a,Ord a)
                => Point V2 a -> Point V2 a -> Point V2 a -> Point V2 a -> Bool
pointInTriangle p0@(P (V2 p0x p0y)) p1@(P (V2 p1x p1y)) p2@(P (V2 p2x p2y)) (P (V2 px py)) =
